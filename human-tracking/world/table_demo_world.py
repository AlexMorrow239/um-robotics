...
class robocanes_isaac_world:
    _coffee_table_counter = 0  # Class-level counter for unique coffee table names
    ...
    def add_coffee_table(self, position, rotation, name=None):
        coffee_table_usd_path = os.path.join(usd_repo_path, 'robocanes_lab', 'robocanes_lab', 'coffeeTable.usd')

        # Generate unique name if not provided
        if name is None:
            robocanes_isaac_world._coffee_table_counter += 1
            coffee_table_name = f"coffee_table_{robocanes_isaac_world._coffee_table_counter}"
        else:
            coffee_table_name = name
        ...
        coffee_table_prim = prims.create_prim(
            ...
            scale=[1, 1, 1],
            semantic_label="coffee_table"  # Keep semantic label consistent for all tables
        )
    ...
    def populate_sim_world(self):
        ...
        self.add_coffee_table(position=[-0.5, 3.0, 0.75], rotation=[0.0, 0.0, 0.0])
        self.add_coffee_table(position=[-3.0, 5.0, 0.75], rotation=[0.0, 0.0, 45.0])
